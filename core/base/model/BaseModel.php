...
abstract class BaseModel extends BaseModelMethods
{
	...
	public function buildUnion($table, $set)
	{
		...
		return $this; // возвращаем указатель на контекст (на текущий объект данного класса), что бы иметь возможность 
		// использовать паттерн(method) chaining Мы сможем при необходимости вызывать метод: buildUnion цепочкой (каждый 
		// метод возвращает объект, позволяя связывать вызовы в одну инструкцию без необходимости использования переменных 
	...
	public function getUnion($set = [])
	{
		...
		foreach ($this->union as $key => $item) {
			...
			if (!empty($item['join'])) {
				...
				foreach ($item['join'] as $table => $data) {
					...
					if (array_key_exists('fields', $data) && $data['fields']) {
						...
						// пробежимся по колонкам и в join добавим (в его ячейку: $key) добавим поля, которые кже есть в join
			...
			// создадим 2-е ячейки в массиве и установим для них первоначальные значения (для удобства обращения):

			// последняя сджойненная таблица
		...
		// сформируем переменную: $order
		$order = $this->createOrder($set);

		// формируем переменную: кол-во необходимых нам записей
		$limit = !empty($set['limit']) ? 'LIMIT ' . $set['limit'] : '';
